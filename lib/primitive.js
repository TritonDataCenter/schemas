...
/* JSSTYLED */
var v4addr_base_re = '(?:(?:\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])\\.){3}(?:\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])';
var v4addr_re = '^' + v4addr_base_re + '$';
var v4cidr_re = '^' + v4addr_base_re + '/(?:\\d|[1-2]\\d|3[0-2])';
var etheraddr_re = '^(?:[0-9a-f][0-9a-f]:){5}[0-9a-f]{2}$';

/*
 * A NAPI Network or Fabric VLAN name. This is somewhat arbitrary. The main
 * goals are to have reasonable length limits and to avoid special and
 * non-ASCII chars.
 */
var netname_re = '^[a-zA-Z][a-zA-Z0-9_\\./-]{1,255}$';
...
var qual = {
	...
	str: {
		...
		v4cidr: {
			desc: 'Is a valid IPv4 subnet in CIDR notation',
			_q: { 'pattern': v4cidr_re }
		},
		hostname: {
		...
		netname: {
			desc: 'Is a valid NAPI network or fabric VLAN name',
			_q: { 'pattern': netname_re }
		}
	},
	intg: {
		pos: {
...
var base = {
	...
	str: {
		...
		v4cidr: {
			_s: schema_util.type('string', qual.str.v4cidr)
		},
		hostname: {
		...
		netname: {
			_s: schema_util.type('string', qual.str.netname)
		}
	},
	intg: {
